...
public class MainController implements Initializable{
    ...
    public boolean elaraRS232 = false;
    ...
    @FXML
    private ComboBox staticQListElara;
    @FXML
    ...
    @FXML
    private RadioButton initQTrue;
    @FXML
    private RadioButton initQFalse;
    @FXML
    private ComboBox initQListElara;
...
//    Alert alert = new Alert(AlertType.INFORMATION);
    ...
    public boolean isElaraCDC = false;
    ...
    public void initialize(URL location, ResourceBundle resources) 
    {
        ...
        setHomeContent();
        showAboutInfo();
        writeDataPane.setVisible(false);
    }
    ...
    public void disableFeatures()
    {
        ...
        staticQListElara.setDisable(true);
        initQTrue.setDisable(true);
        initQFalse.setDisable(true);
        initQListElara.setDisable(true);
...
//    }
    ...
    private void workflowReadWritePower(ActionEvent event) 
    {
        if(workflow.getSelectionModel().getSelectedItem().toString().equalsIgnoreCase("HDR (Single Tag Read Near)"))
        {
            ...
            qEnable.selectToggle(initQTrue);
            initQListElara.setValue("2");
        }
        else if(workflow.getSelectionModel().getSelectedItem().toString().equalsIgnoreCase("Monitor (Bulk Read Far)"))
        {
            readPowerSliderElara.setValue(27.0);
            writePowerSliderElara.setValue(27.0);
            rfReadElara.setText("27.0");
            rfWriteElara.setText("27.0");
        ...
        String workflowSelected = workflow.getValue().toString();
        if(workflowSelected.equalsIgnoreCase("HDR (Single Tag Read Near)") || workflowSelected.equalsIgnoreCase("Monitor (Bulk Read Far)"))
        {
            writeDataPane.setVisible(false);
        }
        else
        {
            writeDataPane.setVisible(true);
        }
    }
...
//                alert.hide();
    ...
    private void getCfgFieldsAllAndSetToUI(ElaraJSONParser p) throws JSONException
    {
        ...
        {
            // parse the response
            HashMap<String,String> initSettingsMap = p.parseGetCfgAllResponse(getCfgFieldsAllResp);
            regionElara.getItems().clear();
            String regionSet = initSettingsMap.get("FreqReg");
            ...
            String gen2Q = initSettingsMap.get("Gen2Q");
            ...
            else if (gen2Q.equals("Static")) 
            {
                ...
                staticQListElara.setValue("");
            }

            boolean gen2QEnable = Boolean.parseBoolean(initSettingsMap.get("Gen2QEnable"));
            if(gen2QEnable == true)
            {
                initQTrue.setSelected(true);
            }
            else
            {
                initQFalse.setSelected(true);
            }

            initQListElara.setValue(initSettingsMap.get("Gen2QInitVal"));
            tariGroupElara.selectToggle(tari6_25usElara);
            ...
            if(isConnected)
            {
                    ...
                    || currentWorkflow.equalsIgnoreCase("Update Tag"))
                {
                    if((currentWorkflow.equalsIgnoreCase("Update Tag")) && (toggleButtonStart.getText().equals("Start")))
                    {
                        ...
                        if(!jsonParser.isCommandSuccess(writeDataResponse))
                        {
                            String errorMessage = jsonParser.errorInfo(writeDataResponse);
                ...
                else if(currentWorkflow.equalsIgnoreCase("Commission Single Tag"))
                {
                    try
                    {
                        ...
                        if(jsonParser.isCommandSuccess(responseReceived))
                        {
                            showWarningErrorMessage("success", "Write data is successful");
                        }
                        else
                        {
                            String errorMessage = jsonParser.errorInfo(responseReceived);
                            showWarningErrorMessage("error", errorMessage);
                        }
                    }
                    catch(Exception ex)
    ...
    private String setWriteDataProfile(ElaraJSONParser jsonParser) throws Exception
    {
        ...
        try
        {
            ...
            String writeProfileSendCmd = jsonParser.setProfileWriteData(memBankToWrite, startAddress, wordCount, retryLimit, dataToWrite, check, isDTselected);
...
//            alert.hide();
    ...
    private void getCfgFieldsModeAndSetToUI(ElaraJSONParser p) throws JSONException
    {
        ...
        if(ejsonp.isCommandSuccess(getCfgWorkflowResponse))
        {
            ...
            boolean gen2QEnable = Boolean.parseBoolean(settingsMap.get("Gen2QEnable"));
            if(gen2QEnable == true)
            {
                initQTrue.setSelected(true);
            }
            ...
            initQListElara.setValue(settingsMap.get("Gen2QInitVal"));
            tariGroupElara.selectToggle(tari6_25usElara);
        }
    }

    void applyConfigurationsToElara()
    {
        if (isConnected) 
        {
            try
            {
                ...
                String buzzerResponse="";
                String autonomousResponse="";
                String readWritePowerResponse="";
                String metadataResponse="";
                String regionResponse="";
                String hidResponse="";
                String outputDataResponse="";
                String workflowResponse = configureWorkflowSettingsToElara(ejsonp);
                if(workflowResponse.equalsIgnoreCase("Success"))
                {
                   buzzerResponse = configureBuzzerTone(ejsonp);
                }
                if(buzzerResponse.equalsIgnoreCase("Success"))
                {
                   autonomousResponse = configureAutonomousRead(ejsonp);
                }
                if(autonomousResponse.equalsIgnoreCase("Success"))
                {
                   readWritePowerResponse = configureReadWritePower(ejsonp, elaraReadPowerValue, elaraWritePowerValue);
                }
                if(readWritePowerResponse.equals("Success"))
                {
                    metadataResponse = configureMetadataFields(ejsonp);
                }
                if(metadataResponse.equalsIgnoreCase("Success"))
                {
                    regionResponse = configureRegion(ejsonp);
                }
                if(!elaraRS232)
                {
                    if(regionResponse.equalsIgnoreCase("Success"))
                    {
                        hidResponse = configureHID(ejsonp);
                    }
                    if(hidResponse.equalsIgnoreCase("Success"))
                    {
                        //ConfigureDataFormat
                        if(hidEnabled && !outputDataFormat.isDisabled())
                        {
                            outputDataResponse = configureOutputDataFormat(ejsonp);
                            resultsTabElaraCDC.setDisable(true);
                            resultsTabElaraHID.setDisable(true);
                        }
                        else
...
//                alert.hide();
                if(elaraRS232)
                {
                    if(buzzerResponse.equalsIgnoreCase("Success") && autonomousResponse.equalsIgnoreCase("Success")
                        && metadataResponse.equalsIgnoreCase("Success") && readWritePowerResponse.equalsIgnoreCase("Success")
                        && regionResponse.equalsIgnoreCase("Success") && workflowResponse.equalsIgnoreCase("Success"))
                    {
                        showWarningErrorMessage("success", "Configurations applied successfully.");
                    }
                    else
                    {
                        if(!workflowResponse.contains("Success") && !workflowResponse.equals(""))
                        {
                            showWarningErrorMessage("error", workflowResponse);
                        }
                        else if(!buzzerResponse.contains("Success") && !buzzerResponse.equals(""))
                        {
                            showWarningErrorMessage("error", buzzerResponse);
                        }
                        else if(!autonomousResponse.contains("Success") && !autonomousResponse.equals(""))
                        {
                            showWarningErrorMessage("error", autonomousResponse);
                        }
                        else if(!readWritePowerResponse.contains("Success") && !readWritePowerResponse.equals(""))
                        {
                            showWarningErrorMessage("error", readWritePowerResponse);
                        }
                        else if(!metadataResponse.contains("Success") && !metadataResponse.equals(""))
                        {
                            showWarningErrorMessage("error", metadataResponse);
                        }
                        else if(!regionResponse.contains("Success") && !regionResponse.equals(""))
                        {
                            showWarningErrorMessage("error", regionResponse);
                        }
                    }
                }
                else
                {
                    if(buzzerResponse.equalsIgnoreCase("Success") && autonomousResponse.equalsIgnoreCase("Success")
                            && metadataResponse.equalsIgnoreCase("Success") && readWritePowerResponse.equalsIgnoreCase("Success")
                            && regionResponse.equalsIgnoreCase("Success") && workflowResponse.equalsIgnoreCase("Success")
                            && hidResponse.equalsIgnoreCase("Success"))
                    {
                        if(hidEnabled && !outputDataFormat.isDisabled() && outputDataResponse.equalsIgnoreCase("Success"))
                        {
                            showWarningErrorMessage("success", "Configurations applied successfully.");
                        }
                        else
                    ...
                    else
                    {
                        ...
                        else if(!hidResponse.contains("Success") && !hidResponse.equals(""))
                        {
                            showWarningErrorMessage("error", hidResponse);
                        }
                        else if(!outputDataResponse.contains("Success") && !outputDataResponse.equals(""))
                        {
                            showWarningErrorMessage("error", outputDataResponse);
                        }
                    }
                }
            }
    ...
    private String configureWorkflowSettingsToElara(ElaraJSONParser p) throws JSONException
    {
        ...
        {
            workflowSuccess = (String)workflowJSON.get("ErrInfo");
        }
        return workflowSuccess;
    }

    private String configureBuzzerTone(ElaraJSONParser p) throws JSONException
    {
        String buzzerSuccess;
        ...
        {
            buzzerSuccess = "Success";
        }
        else
        {
            buzzerSuccess = (String)buzzerJSON.get("ErrInfo");
        }
        return buzzerSuccess;
    }
    
    private String configureOutputDataFormat(ElaraJSONParser p) throws JSONException
    {
        String dataFormatSuccess;
        ...
        {
            dataFormatSuccess = "Success";
        }
        else
        {
            dataFormatSuccess = (String)outputDataFormatJSON.get("ErrInfo");
        }
        return dataFormatSuccess;
    }

    private String configureAutonomousRead(ElaraJSONParser p) throws JSONException
    {
        String autonomousSuccess;
        ...
        {
            autonomousSuccess = "Success";
        }
        else
        {
            autonomousSuccess = (String)autonomousJSON.get("ErrInfo");
        }
        return autonomousSuccess;
    }

    private String configureReadWritePower(ElaraJSONParser p, String readPowerValue, String writePowerValue) throws JSONException
    {
        String readWritePowerSuccess;
        StringBuffer sb = new StringBuffer();
        sb.append(readPowerValue);
        String setReadPowerCommand = p.customJSONCommand(ECTConstants.SET_RZ_READ_POWER, sb);
        String responseReceived = sendMessage(sp, setReadPowerCommand, elaraTransportListener);
        JSONObject readWritePowerJSON = new JSONObject(responseReceived);
        if(readWritePowerJSON.get("Report").equals("SetRZ") && readWritePowerJSON.get("ErrID").equals(0))
        {
            rfReadElara.setText(readPowerValue);
            readPowerSliderElara.setValue(Double.parseDouble(readPowerValue));
            rfWriteElara.setText("23.0");
            writePowerSliderElara.setValue(23.0);
            readWritePowerSuccess = "Success";
        }
        else
        {
            readWritePowerSuccess = (String)readWritePowerJSON.get("ErrInfo");
        }
        return readWritePowerSuccess;
    }
    
    private String configureRegion(ElaraJSONParser p) throws JSONException
    {
        String regionSuccess;
        ...
        {
            regionSuccess = "Success";
        }
        else
        {
            regionSuccess = (String)regionJSON.get("ErrInfo");
        }
        return regionSuccess;
    }
    
    private String configureMetadataFields(ElaraJSONParser p) throws JSONException
    {
        ...
        {
            metadataSuccess = (String)metadataJSON.get("ErrInfo");
        }
    ...
    private Runnable updateFirmwareThread = new Runnable()
    {
        ...
        public void run()
        {
            try
            {
                ...
                {
                    configureTabElara.setDisable(true);
                    firmwareUpdateSuccess = elaraFirmwareUpgrade(sp,firmwareFile);
                    if(firmwareUpdateSuccess)
                    {
            ...
            if(inBootLoaderMode)
            {
                ...
                if(spFWUpgrade.openPort(3000))
                {
                    ...
                    while(inputStream.read(b)!= -1)
                    {
                        ...
                        System.out.println("Firmware update in progress "+String.format("%.1f", percentComplete)+"%");
                        count++;
                        if(err!=0){
   ...
   private void checkReadWritePowerOnUSBProModule()
   {
    ...
    public void cleanUpResources()
    {
        ...
        for (int i = 0; i < portList.length; i++)
        {
            ...
            {
                String deviceName = portList[i].getDescriptivePortName();
                openPort = deviceName.substring(deviceName.indexOf("(") + 1, deviceName.indexOf(")"));
            }
        }
        SerialPort spToBeClosed = SerialPort.getCommPort(openPort);
        if(spToBeClosed!=null)
        {
            spToBeClosed.closePort();
        }
    }
    ...
    private String configureHID(ElaraJSONParser ejsonp) throws JSONException 
    {
        String hidSuccess = "";
        if(elaraHIDModeEnabled.isSelected())
        {
            ...
            {
                hidSuccess = (String)hidJSON.get("ErrInfo");
            }
        }
        else
        {
